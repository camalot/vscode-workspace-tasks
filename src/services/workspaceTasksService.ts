...
export class WorkspaceTasksService {
    ...
    private constructor() {
        this.config = JSON.parse(JSON.stringify(fileTasksData)); // Deep copy initial config
    ...
    private async loadWorkspaceConfig() {
        const newConfig: FileTasksConfig = JSON.parse(JSON.stringify(fileTasksData));
        // Use recursive glob to find file anywhere in workspace
        const files = await vscode.workspace.findFiles('**/.workspace-tasks.json', '**/node_modules/**');
        if (files.length > 0) {
            for (const file of files) {
                try {
                    ...
                    const localConfig = JSON.parse(jsonString) as FileTasksConfig;
