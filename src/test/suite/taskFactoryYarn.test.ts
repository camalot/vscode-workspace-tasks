...
suite('Task Factory Yarn Test Suite', () => {
  ...
  test('uses package.json directory as cwd, not workspace root', async function () {
    ...
    const item = new TaskItem('test-script', vscode.TreeItemCollapsibleState.None, 'yarn', uri);
  ...
  test('constructs correct yarn run command', async function () {
    const rootPath =
      vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0
        ? vscode.workspace.workspaceFolders[0].uri.fsPath
        : path.join(__dirname, 'test-workspace');
    ...
    const item = new TaskItem('build', vscode.TreeItemCollapsibleState.None, 'yarn', uri);
    item.taskFileUri = uri;

    const created = await createTaskForItem(item);

    assert.ok(created, 'Should create a task');
    assert.ok(created?.command?.includes('yarn'), `Command should contain 'yarn' but was: ${created?.command}`);
