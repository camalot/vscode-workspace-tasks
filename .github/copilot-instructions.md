...
Always run the tests using `pnpm test` after making changes, and adding new tests for any new code you write. If only updating documentation or non-code files, running tests may not be necessary.
...
- Goal for 95% or greater code coverage for a file. New code should have 100% coverage.
  - Use `pnpm run vscode:test:coverage` to check the full coverage report.
  - Use `pnpm run vscode:test:coverage -- --run=out/test/suite/testFile1.ts --run=out/test/suite/testFile2.ts` to check the coverage for a specific files.
