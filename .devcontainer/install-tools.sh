...
# populate that path.
ACT_VERSION="0.2.72"  # bump as needed; use a known working release
ACT_PATH="tools/act/act"

if [ ! -f "$ACT_PATH" ]; then
  echo "Installing act v${ACT_VERSION}..."
  mkdir -p "$(dirname "$ACT_PATH")"
  curl -fsSL "https://github.com/nektos/act/releases/download/v${ACT_VERSION}/act_linux_amd64.tar.gz" \
    | tar -xz -C "$(dirname "$ACT_PATH")"
  chmod +x "$ACT_PATH"
else
